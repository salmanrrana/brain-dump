...
export async function commitAndShip(
  ...
  const prArgs = ["pr", "create", "--title", input.prTitle.trim(), "--body", input.prBody];
  if (input.draft) {
    prArgs.push("--draft");
  }

  const prCreateResult = await deps.execFileNoThrow("gh", prArgs, commandOptions);
...
const defaultCommitAndShipDeps: CommitAndShipDeps = {
  db: sqlite,
  execFileNoThrow,
...
export const generatePrBodyServerFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }: { data: GeneratePrBodyInput }): Promise<GeneratePrBodyResult> => {
    return generatePrBody(data, sqlite);
  });
