...
export interface EpicCodeChangesSectionProps {
  epicId: string;
...
/**
 * Aggregated, non-blocking "Code changes" entry point for epic detail.
 *
 * The summary loads lazily after page paint (it is never fetched in the route
 * loader). The compact state answers "are there changes across the epic, how
 * many files, how big" before the user opens the full review surface. Inside the
 * surface, the user can switch between All tickets and a single ticket. Full
 * patch text is only fetched once the panel is open and a file is selected.
 */
export function EpicCodeChangesSection({
  epicId,
  prUrl,
  prNumber,
  branchName,
  ...
  // keep open + view preferences) whenever the epic id changes.
  const previousEpicId = useRef(epicId);
  useEffect(() => {
    if (previousEpicId.current === epicId) {
      return;
    }
    previousEpicId.current = epicId;
  ...
  return (
    <section
      ...
      aria-labelledby="epic-code-changes-heading"
      data-testid="epic-code-changes-section"
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <button
          ...
          aria-expanded={open}
          aria-controls="epic-code-changes-surface"
      ...
      {open && (
        <div id="epic-code-changes-surface" className="mt-4">
