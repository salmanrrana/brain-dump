...
export interface SecretDetectionResult {
  ...
  detected: boolean;
  /** Array of detected secret type names (no actual values) */
  types: string[];
 ...
 * @returns Detection result with types but not values
 *
 * @example
 ...
 * @param placeholder - Replacement text for secrets
 * @returns Content with secrets replaced by placeholder
 */
export function redactSecrets(content: string, placeholder = "[REDACTED]"): string {
  if (!content || typeof content !== "string") {
    return content;
  }
