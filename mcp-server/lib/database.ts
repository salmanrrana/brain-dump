...
export function migrateFromLegacySync(): MigrationResult {
  ...
  try {
    ...
    try {
      ...
      log.error("Failed to create backup, continuing anyway", err);
    }
...
export function runMigrations(db: Database.Database): void {
  ...
  try {
    const columns = db.prepare("PRAGMA table_info(tickets)").all() as ColumnInfo[];
    ...
    log.error("Failed to check/add linked_commits column", error);
  }
  ...
  try {
    ...
    const error = err instanceof Error ? err : new Error(String(err));
    log.error("Failed to check/add working_method column", error);
  }
  ...
  try {
    ...
    const error = err instanceof Error ? err : new Error(String(err));
    log.error("Failed to check/add git/PR tracking columns", error);
  }
  ...
  try {
    ...
    const error = err instanceof Error ? err : new Error(String(err));
    log.error("Failed to create ralph_events table", error);
  }
  ...
  try {
    ...
    if (tables.length === 0) {
      ...
      const columns = db.prepare("PRAGMA table_info(ralph_sessions)").all() as ColumnInfo[];
      ...
      try {
        ...
        if (!error.message?.includes("already exists")) {
          ...
          log.warn("Failed to create idx_ralph_sessions_state index", logErr);
        }
      }
    }
  } catch (err) {
    const error = err instanceof Error ? err : new Error(String(err));
    log.error("Failed to create/migrate ralph_sessions table", error);
  }
  ...
  try {
    ...
    const error = err instanceof Error ? err : new Error(String(err));
    log.error("Failed to create conversation_sessions table", error);
  }
  ...
  try {
    ...
    const error = err instanceof Error ? err : new Error(String(err));
    log.error("Failed to create conversation_messages table", error);
  }
  ...
  try {
    ...
    const error = err instanceof Error ? err : new Error(String(err));
    log.error("Failed to create audit_log_access table", error);
  }

  // Add conversation logging settings columns if missing
  try {
    const settingsColumns = db.prepare("PRAGMA table_info(settings)").all() as ColumnInfo[];
    ...
    const error = err instanceof Error ? err : new Error(String(err));
    log.error("Failed to add conversation logging settings columns", error);
  }
}
...
export function initDatabase(dbPath?: string): InitDatabaseResult {
  ...
  if (!existsSync(xdgDbPath)) {
    if (existsSync(legacyDbPath) && !isMigrationComplete()) {
      ...
      throw new Error(
        `Database not found at ${xdgDbPath} or ${legacyDbPath}. Run Brain Dump first to create the database.`
      );
    }
  }
