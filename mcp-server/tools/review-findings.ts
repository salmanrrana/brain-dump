...
import {
  ...
  logMcpCallEvent,
  withTelemetry,
} from "../lib/telemetry-self-log.js";
...
const FINDING_UPDATE_STATUSES = ["fixed", "wont_fix", "duplicate"] as const;

// ============================================
// Type Definitions
// ============================================

/** DB row for ticket_workflow_state table */
interface DbTicketWorkflowState {
  id: string;
  ticket_id: string;
...
/** DB row for review_findings table */
 ...
 */
export function registerReviewFindingsTools(server: McpServer, db: Database.Database): void {
...
Returns the created finding with ID and current counts.`,
    ...
    withTelemetry(
      ...
      async (params: unknown) => {
        ...
        let workflowState = db
          .prepare("SELECT * FROM ticket_workflow_state WHERE ticket_id = ?")
          .get(ticketId) as DbTicketWorkflowState | undefined;
        if (!workflowState) {
          const id = randomUUID();
          ...
          workflowState = db
            .prepare("SELECT * FROM ticket_workflow_state WHERE ticket_id = ?")
            .get(ticketId) as DbTicketWorkflowState;
        }
        ...
        const commentContent = `Review finding: ${emoji} [${severity}] ${category}\n\n${description}${filePath ? `\n\nFile: ${filePath}${lineNumber ? `:${lineNumber}` : ""}` : ""}${suggestedFix ? `\n\nSuggested fix:\n${suggestedFix}` : ""}`;
...
Returns the updated finding.`,
    {
      ...
      status: z.enum(FINDING_UPDATE_STATUSES).describe("New status"),
    ...
    async ({
      ...
      findingId: string;
      status: (typeof FINDING_UPDATE_STATUSES)[number];
      fixDescription?: string | undefined;
    }) => {
      const finding = db.prepare("SELECT * FROM review_findings WHERE id = ?").get(findingId) as
        | DbReviewFindingRow
        | undefined;
      if (!finding) {
        return {
          content: [{ type: "text" as const, text: `Finding not found: ${findingId}` }],
          isError: true,
        };
      }
      ...
      const ticket = db.prepare("SELECT * FROM tickets WHERE id = ?").get(finding.ticket_id) as
        | DbTicket
        | undefined;
      if (!ticket) {
        ...
        return {
          content: [{ type: "text" as const, text: `Ticket not found for this finding` }],
          isError: true,
        };
      }
      ...
      if (status === "fixed") {
        ...
        const openCriticalMajor = db
          ...
          .get(finding.ticket_id) as { count: number };
      ...
      // Create progress comment (per spec: mandatory audit trail)
      const statusEmoji: Record<string, string> = { fixed: "✅", wont_fix: "⏭️", duplicate: "📋" };
      const emoji = statusEmoji[status] || "📝";
      const contentLines = [
        `${emoji} Finding marked as ${status}`,
...
Returns array of findings.`,
    ...
    withTelemetry(
      ...
      (params) => (params as { ticketId: string }).ticketId,
      async (params: unknown) => {
        const { ticketId, status, severity, agent } = params as {
          ticketId: string;
          status?: (typeof FINDING_STATUSES)[number] | undefined;
        ...
        const findings = db.prepare(query).all(...queryParams) as DbReviewFindingRow[];
    ...
    withTelemetry(
      ...
      async (params: unknown) => {
        ...
        const openMinor = findings.filter(
          (f) => f.severity === "minor" && f.status === "open"
        ).length;
