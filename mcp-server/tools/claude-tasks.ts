...
/** DB row for a minimal ticket lookup */
...
/** Parsed Ralph state result */
interface RalphStateResult {
  ticketId: string | null;
  sessionId: string | null;
}

/** DB row for an existing Claude task */
...
/** Formatted task for get_claude_tasks response */
 ...
 */
function readRalphState(projectPath?: string | undefined): RalphStateResult {
  const paths: string[] = [
  ...
  for (const statePath of paths) {
    if (existsSync(statePath)) {
      try {
        ...
        log.warn(`Failed to read Ralph state from ${statePath}: ${errorMsg}`);
      }
    }
  }
 ...
 */
function ensureTicketExists(db: Database.Database, ticketId: string): TicketBasicRow | undefined {
  return db.prepare("SELECT id, title FROM tickets WHERE id = ?").get(ticketId) as
    | TicketBasicRow
    | undefined;
}
 ...
 */
export function registerClaudeTasksTools(server: McpServer, db: Database.Database): void {
  // Save Claude tasks
  server.tool(
    "save_claude_tasks",
...
Returns the saved tasks.`,
    ...
    async ({
      ...
      if (!tasks || tasks.length === 0) {
        return {
          content: [
            { type: "text" as const, text: "No tasks provided. Send at least one task to save." },
          ],
          isError: true,
        };
      }
      ...
      const transaction = db.transaction(() => {
        ...
        const existingTasks = db
          ...
          .all(resolvedTicketId) as DbClaudeTaskRow[];
        ...
        for (let i = 0; i < tasks.length; i++) {
          ...
          if (existing?.status_history) {
            try {
              ...
              const errorMsg = err instanceof Error ? err.message : String(err);
              log.error(`Failed to parse status_history for task ${taskId}: ${errorMsg}`);
              statusHistory = [];
            }
          }
      ...
      try {
        ...
        const statusCounts: Record<string, number> = savedTasks.reduce(
          (acc, t) => {
...
${savedTasks.map((t, i) => `${i + 1}. ${getTaskStatusIcon(t.status)} ${t.subject}`).join("\n")}`,
        ...
        const errorMsg = err instanceof Error ? err.message : String(err);
        log.error(`Failed to save Claude tasks: ${errorMsg}`);
        return {
          content: [{ type: "text" as const, text: `Failed to save tasks: ${errorMsg}` }],
          isError: true,
        };
      }
    }
...
Returns array of tasks with full details.`,
    ...
    async ({
      ...
      `
        ...
        .all(resolvedTicketId) as DbClaudeTaskFullRow[];
      ...
      const formattedTasks: FormattedTask[] = tasks.map((t) => {
        ...
        if (includeHistory && t.status_history) {
          try {
            ...
            const errorMsg = err instanceof Error ? err.message : String(err);
            log.warn(`Failed to parse status_history for task ${t.id}: ${errorMsg}`);
            task.statusHistory = [];
          }
        }
      ...
      const statusCounts: Record<string, number> = formattedTasks.reduce(
        (acc, t) => {
          acc[t.status] = (acc[t.status] || 0) + 1;
          return acc;
        },
...
Returns confirmation of cleared tasks.`,
    ...
    async ({ ticketId }: { ticketId?: string | undefined }) => {
      // Auto-detect ticket from Ralph state if not provided
      let resolvedTicketId = ticketId;
      let resolvedSessionId: string | null = null;
      ...
      `
        ...
        .all(resolvedTicketId) as SavedTask[];
      ...
      if (existingTasks.length === 0) {
        return {
          content: [
            {
              type: "text" as const,
              text: `No Claude tasks to clear for ticket "${ticket.title}".`,
            },
          ],
        };
      }
    ...
    async ({ ticketId, limit = 10 }: { ticketId: string; limit?: number | undefined }) => {
      ...
      const formattedSnapshots: ParsedSnapshot[] = snapshots.map((s) => {
        ...
        try {
          ...
          const errorMsg = err instanceof Error ? err.message : String(err);
          log.warn(`Failed to parse tasks snapshot ${s.id}: ${errorMsg}`);
        }
        return {
          id: s.id,
