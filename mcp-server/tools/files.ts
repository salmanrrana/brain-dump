...
/** Row shape returned by the ticket + project join query */
 ...
 */
export function registerFileTools(server: McpServer, db: Database.Database): void {
...
Returns:
  Updated list of linked files for the ticket.`,
    ...
    async ({ ticketId, files }: { ticketId: string; files: string[] }) => {
      if (files.length === 0) {
      ...
      if (ticket.linked_files) {
        try {
          linkedFiles = JSON.parse(ticket.linked_files) as string[];
        } catch {
          linkedFiles = [];
        }
      }
...
Returns:
  Array of tickets that have this file linked.`,
    ...
    async ({ filePath, projectId }: { filePath: string; projectId?: string | undefined }) => {
      ...
      const allTickets = db.prepare(query).all(...params) as TicketWithProject[];

      const matchingTickets = allTickets.filter((ticket) => {
        try {
          const linkedFiles = JSON.parse(ticket.linked_files!) as string[];
      ...
      const results = matchingTickets.map((t) => ({
        ...
        linkedFiles: JSON.parse(t.linked_files!) as string[],
      }));
