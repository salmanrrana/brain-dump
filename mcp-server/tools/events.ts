 ...
 */
export function registerEventTools(server, db) {
  // Emit Ralph event
  server.tool(
    "emit_ralph_event",
  ...
  The created event with its ID and timestamp.`,
    ...
    async ({ sessionId, type, data }) => {
      ...
      try {
        ...
        log.error(`Failed to emit event: ${err.message}`, { sessionId, type });
        return {
          content: [{
            type: "text",
            text: `Failed to emit event: ${err.message}`,
          }],
          isError: true,
        };
      }
  ...
  Array of events with their types, data, and timestamps.`,
    ...
    async ({ sessionId, since, limit = 50 }) => {
      try {
        ...
        log.error(`Failed to get events: ${err.message}`, { sessionId });
        return {
          content: [{
            type: "text",
            text: `Failed to get events: ${err.message}`,
          }],
          isError: true,
        };
      }
  ...
  Number of events deleted.`,
    ...
    async ({ sessionId }) => {
      try {
        ...
        log.error(`Failed to clear events: ${err.message}`, { sessionId });
        return {
          content: [{
            type: "text",
            text: `Failed to clear events: ${err.message}`,
          }],
          isError: true,
        };
      }
