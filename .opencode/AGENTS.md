...
Three core agents are available:

- **ralph** — Autonomous ticket implementation agent (works through backlogs)
- **planner** — Designs implementation plans for complex features
- **ticket-worker** — Implements individual tickets with human guidance
...
Ralph's workflow instructions are generated by `getRalphPrompt()` in `src/api/ralph-prompts.ts`.
...
Telemetry is handled by MCP self-instrumentation — no external plugin needed.
