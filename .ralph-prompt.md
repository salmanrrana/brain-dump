...
6. Implement the feature:
   - Update state to 'analyzing': update_session_state({ state: 'analyzing' })
   - Update state to 'implementing': update_session_state({ state: 'implementing' })
   - Write the code
   - Update state to 'testing': update_session_state({ state: 'testing' })
   - Run tests: pnpm test (or npm test)
   - Verify acceptance criteria
7. Git commit: git commit -m "feat(<ticket-id>): <description>"
   - Update state to 'committing': update_session_state({ state: 'committing' })
8. Call complete_ticket_work(ticketId, "summary of changes")
   - This moves ticket to 'ai_review' status
9. Run AI review phase (if no critical issues skip this):
   - Update state to 'reviewing': update_session_state({ state: 'reviewing' })
   - Run review agents (manual or via skill)
   - Submit findings: submit_review_finding()
   - Mark fixed: mark_finding_fixed()
   - STOP if critical/major findings remain unfixed
10. Generate demo script (if all critical/major findings fixed):
    - Call generate_demo_script() with demo steps
    - This moves ticket to 'human_review'
    - STOP and wait for human approval
11. Complete session: complete_ralph_session(sessionId, "success")
12. If all tickets complete or in human_review, output: PRD_COMPLETE
