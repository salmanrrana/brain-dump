...
6. **Implement** - Write code, verify acceptance criteria
7. **Run tests** - `pnpm type-check && pnpm lint && pnpm test` (all must pass)
8. **Commit** - `git commit -m "feat(<ticket-id>): <description>"`
9. **Complete implementation** - Call `complete_ticket_work(ticketId, "summary")` → moves to **ai_review**

### Phase 2: AI Review (REQUIRED)

10. **Run review agents** - Launch all 3 in parallel: code-reviewer, silent-failure-hunter, code-simplifier
11. **Submit findings** (if any) - `submit_review_finding({ ticketId, agent, severity, category, description })`
12. **Fix critical/major** (if any) - Then `mark_finding_fixed({ findingId, status: "fixed" })`
13. **Verify complete** - `check_review_complete({ ticketId })`
    - If response shows `canProceedToHumanReview: false`: **STOP and fix remaining critical/major findings**
    - If response shows `canProceedToHumanReview: true`: Continue to Phase 3

### Phase 3: Demo Generation

14. **Generate demo** - `generate_demo_script({ ticketId, steps: [...] })` → moves to **human_review**

### Phase 4: STOP

15. **Complete session** - `complete_ralph_session(sessionId, "success")`
16. **STOP** - Human must approve via `submit_demo_feedback`. Never auto-complete.
...
2. **Update state** as you transition through phases:
   ...
   ```
   update_session_state({ sessionId: "<sessionId>", state: "analyzing" })
   ```
...
| committing → reviewing            | AI review phase        | Running review agents, fixing findings |
