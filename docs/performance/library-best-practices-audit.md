...
| Reuse prepared statements on hot paths (Drizzle `.prepare()`/`sql.placeholder()` or module-level cached `sqlite.prepare()`).         | No persistent reuse. `getTicketSummaries` (`src/api/tickets.ts:439-444`) is a Drizzle `db.select()` builder that re-plans SQL on every call (no `.prepare('name')`); `searchTickets` (`src/api/search.ts:75,113`), tags (`src/api/tags.ts:48,101`), and telemetry analytics 6× (`src/api/telemetry.ts:357,375,400,415,439,468`) each call raw `sqlite.prepare()` inside the handler. | medium   | SQLite Data Layer Speed | —         |
...
| ✅ SQL-side aggregation (no N+1).                                                                                                    | `loadDashboardAnalytics` uses GROUP BY/CASE/AVG throughout (`src/api/analytics.ts:77-421`; wrapped by `getDashboardAnalytics` at `:428`); `getProjectsWithEpics` single LEFT JOIN (`src/api/projects.ts:52-69`).                                                                                                                                                                     | —        | none                    | —         |
