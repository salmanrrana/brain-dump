...
- `epic_continuation_jobs` uses the shared helper through `core/epic-continuation.ts` to serialize failed-verification Ralph continuations.

Verification jobs renew their lease while the runner is active. The default app process does not execute jobs from its possibly stale module graph; it supervises the queue and spawns a fresh one-shot drain when a job is ready or a running lease has expired. Claim and settlement ownership checks remain authoritative if an old worker returns after losing its lease.
