...
#
# Copilot CLI provider adapter
# Installs workflow-pack to ~/.copilot/brain-dump/ with hooks.json references
#
# Usage:
...
case "$ACTION" in
  enable|disable|status)
    echo "Copilot CLI adapter: $ACTION — not yet implemented (see ticket 3.3)"
    exit 0
    ;;
  *)
