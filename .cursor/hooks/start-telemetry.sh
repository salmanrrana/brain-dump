#!/bin/bash
# start-telemetry.sh
# Cursor sessionStart hook for telemetry
#
...
# Use CURSOR_PROJECT_DIR for reliable path resolution (Cursor-specific env var)
...
echo "[$(date -Iseconds)] Cursor sessionStart hook triggered" >> "$LOG_FILE"
...
# Try to detect ticket from Ralph state file (check both .cursor and .claude locations)
TICKET_ID=""

RALPH_STATE="$PROJECT_DIR/.cursor/ralph-state.json"
if [[ -f "$RALPH_STATE" ]]; then
  TICKET_ID=$(jq -r '.ticketId // ""' "$RALPH_STATE" 2>/dev/null || echo "")
fi

# Fall back to Claude state if Cursor state not found
if [[ -z "$TICKET_ID" ]]; then
...
if [[ -n "$TICKET_ID" ]]; then
  echo "[$(date -Iseconds)] Found ticket: $TICKET_ID" >> "$LOG_FILE"
fi

# Output notification to tell Cursor AI to start telemetry
if [[ -n "$TICKET_ID" ]]; then
  echo ""
  echo "╔══════════════════════════════════════════════════════════════╗"
