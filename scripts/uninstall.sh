...
#
# Usage:
#   ./scripts/uninstall.sh              # Remove IDE configs (keeps data)
#   ./scripts/uninstall.sh --vscode     # Remove VS Code integration only
#   ./scripts/uninstall.sh --claude     # Remove Claude Code integration only
#   ./scripts/uninstall.sh --cursor     # Remove Cursor integration only
#   ./scripts/uninstall.sh --opencode   # Remove OpenCode integration only
#   ./scripts/uninstall.sh --sandbox    # Remove Claude Code sandbox configuration
#   ./scripts/uninstall.sh --devcontainer # Remove devcontainer Docker volumes
#   ./scripts/uninstall.sh --all        # Remove everything (including data)
...
remove_opencode() {
    ...
    if [ ! -d "$OPENCODE_DIR" ]; then
        print_info "OpenCode directory not found (.opencode/)"
        SKIPPED+=("OpenCode (not configured)")
        return 0
    fi

    # Remove brain-dump from OpenCode MCP config
    if [ -f "$OPENCODE_CONFIG" ] && grep -q '"brain-dump"' "$OPENCODE_CONFIG"; then
        if command -v node >/dev/null 2>&1; then
            node -e "
const fs = require('fs');
try {
...
" 2>/dev/null && print_success "Removed brain-dump from OpenCode MCP config" && REMOVED+=("OpenCode MCP server")
            ...
            print_info "Manually edit: $OPENCODE_CONFIG"
    ...
    if [ -d "$OPENCODE_DIR/agent" ]; then
        for agent in ralph ticket-worker planner code-reviewer silent-failure-hunter code-simplifier code-reviewer-fallback code-simplifier-fallback; do
        ...
        print_info "OpenCode agent directory not found"
    fi

    # Remove Brain Dump skills
    local skills_removed=0
    if [ -d "$OPENCODE_DIR/skill" ]; then
        for skill in brain-dump-tickets ralph-workflow review review-aggregation tanstack-errors tanstack-forms tanstack-mutations tanstack-query tanstack-types; do
            if [ -d "$OPENCODE_DIR/skill/$skill" ]; then
                rm -rf "$OPENCODE_DIR/skill/$skill"
                skills_removed=$((skills_removed + 1))
            fi
        done
        if [ $skills_removed -gt 0 ]; then
        ...
        print_info "OpenCode skill directory not found"
    fi
}
...
show_help() {
    ...
    echo "  OpenCode:      MCP config, agents, skills in .opencode/"
